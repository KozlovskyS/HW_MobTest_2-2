...
class ChangeTextTest {
    ...
    private val textToNewActivity = "New Activity"
    ...
    fun testEmptyText() {
        ...
        val expected =
            device.findObject(By.res(packageName, "textToBeChanged")).text //сохранение пустое поле

        device.findObject(By.res(packageName, "userInput")).text = textToSetEmpty
        device.findObject(By.res(packageName, "buttonChange")).click()

        val result =
            device.findObject(By.res(packageName, "textToBeChanged")).text  //измененное поле
        assertEquals(result, expected)
    }

    @Test
    fun testTextOpenInNewActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
